...
export function getProspectContext(company: string) {
  const prospect = prospects.find((item) => item.company.toLowerCase() === company.trim().toLowerCase());
  if (!prospect) return null;
  return {
    found: true,
    company: prospect.company,
    industry: prospect.industry ?? "Demo business",
    companySummary: prospect.companySummary,
    recentSignal: prospect.recentSignal ?? prospect.opportunity,
    verifiedFacts: [prospect.companySummary, prospect.opportunity, prospect.personalizationAngle],
  };
}
